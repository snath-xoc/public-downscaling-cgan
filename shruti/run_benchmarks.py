...
parser.add_argument("--log_folder", type=str, help="directory to store results")
parser.add_argument("--predict_year", type=int, help="year to predict on", default=2020)
parser.add_argument(
    "--num_images", type=int, help="number of images to predict on", default=256
)
parser.set_defaults(include_nn_interp=True)
parser.set_defaults(include_zeros=True)
parser.add_argument(
    "--include_nn_interp",
    dest="include_nn_interp",
    action="store_true",
...
log_line(
    log_fname,
    "Model CRPS CRPS_max_4 CRPS_max_16 CRPS_avg_4 CRPS_avg_16 RMSE EMRMSE RALSD MAE",
)
...
for benchmark in benchmark_methods:
    ...
    for ii in range(num_images):
        ...
        if benchmark == "nn_interp":
            sample_benchmark = benchmarks.nn_interp_model(
                inp["lo_res_inputs"][..., tpidx], ds_fac
            )
        elif benchmark == "zeros":
            sample_benchmark = benchmarks.zeros_model(
                inp["lo_res_inputs"][..., tpidx], ds_fac
            )
        else:
        ...
        for method in CRPS_pooling_methods:
            ...
            crps_score = crps_ensemble(
                np.squeeze(truth_pooled, axis=-1), sample_benchmark_pooled
            )
        ...
        if benchmark == "zeros":
            ...
            ralsd_score = calculate_ralsd_rmse(
                np.squeeze(truth, axis=-1), [np.squeeze(sample_benchmark, axis=-1)]
            )
    ...
    log_line(
        log_fname,
        f"{benchmark} {CRPS_pixel:.6f} {CRPS_max_4:.6f} {CRPS_max_16:.6f} {CRPS_avg_4:.6f} {CRPS_avg_16:.6f} {rmse:.6f} {emrmse:.6f} {ralsd:.6f} {mae:.6f}",
    )
